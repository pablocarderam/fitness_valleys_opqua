...
# the fitness valley (number of alleles that affect fitness)
def fitnessLandscape(genome,num_alleles=8):
    num_b = genome[0:num_alleles].count('B')
    if num_b < min(3,num_alleles):
        f = 1 / ( 2 * ( (num_b+1)**5 ) )
    else:
        f = 1 / ( ( num_alleles-num_b+1 )**3 )
...
num_replicates = 32 # per contact rate
...
dat_2 = runFitness(
    lambda g: fitnessLandscape(g,num_alleles=2),
    lambda g: drugCheck(g,num_alleles=2),
    lambda m: stopSimIfResistant(m,num_alleles=2)
    )

dat_4 = runFitness(
    lambda g: fitnessLandscape(g,num_alleles=4),
    lambda g: drugCheck(g,num_alleles=4),
    lambda m: stopSimIfResistant(m,num_alleles=4)
    )

dat_6 = runFitness(
    lambda g: fitnessLandscape(g,num_alleles=6),
    lambda g: drugCheck(g,num_alleles=6),
    lambda m: stopSimIfResistant(m,num_alleles=6)
    )

dat_8 = runFitness(
    lambda g: fitnessLandscape(g,num_alleles=8),
    lambda g: drugCheck(g,num_alleles=8),
    lambda m: stopSimIfResistant(m,num_alleles=8)
    )
...
dat = pd.concat([dat_2,dat_4,dat_6,dat_8])
dat.to_csv('contact_sweep_survival_length_full.csv')
# ...until here

dat = pd.concat([
    pd.read_csv('contact_sweep_survival_length_full.csv')
    ])
...
ax = sns.lineplot(
    x = 'contact_rate_host_host', y='Alive', hue='length', data=dat,
    palette=CB_PALETTE[4:7]+[CB_PALETTE[0]], marker='o',ci=95
    )
...
plt.savefig('FigS1-valley_length.png', bbox_inches='tight')
